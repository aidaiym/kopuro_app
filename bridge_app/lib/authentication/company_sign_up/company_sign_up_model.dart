...
class CompanySignUpModel extends FlutterFlowModel<CompanySignUpWidget> {
  ...
  final textFieldMask9 = MaskTextInputFormatter(mask: '+971-##-###-####');
  ...
  String? _textController9Validator(BuildContext context, String? val) {
    ...
    if (!RegExp(r'^\+971-\d{2}-\d{3}-\d{4}$').hasMatch(val)) {
  ...
  String? _textController12Validator(BuildContext context, String? val) {
    ...
    if (!RegExp('^\\d+\$').hasMatch(val)) {
      return 'The foundation year cannot be negative. Please enter a valid year.';
    }
    return null;
  }
